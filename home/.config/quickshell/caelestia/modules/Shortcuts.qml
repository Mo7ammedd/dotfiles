...
Scope {
    ...
    // Open the launcher's thumbnail wallpaper mode directly.
    // Adapted from dim-ghub/midnight-shell's Caelestia fork.
    // qmllint disable unresolved-type
    CustomShortcut {
        // qmllint enable unresolved-type
        name: "wallpaper"
        ...
        onPressed: {
            if (root.hasFullscreen)
                return;
            ShellState.launcherInitialSearch = `${GlobalConfig.launcher.actionPrefix}wallpaper `;
    ...
    IpcHandler {
        function openSearch(query: string): void {
            ...
            if (state.launcher && search) {
                search.text = query;
                search.forceActiveFocus();
                ShellState.launcherInitialSearch = "";
            } else {
                ShellState.launcherInitialSearch = query;
                state.launcher = true;
            }
        }

        function openWallpaper(): void {
            openSearch(`${GlobalConfig.launcher.actionPrefix}wallpaper `);
        }

        function openClipboard(): void {
            openSearch(`${GlobalConfig.launcher.actionPrefix}clipboard `);
        }

        function searchText(): string {
            return ShellState.componentsForActive()?.find("launcherSearch")?.text ?? "";
        }
