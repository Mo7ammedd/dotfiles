...
StyledListView {
    ...
    model: ScriptModel {
        ...
        onValuesChanged: {
            root.currentIndex = 0;
            root.expandedClipId = -1;
        }
    }

    spacing: Tokens.spacing.small
    orientation: Qt.Vertical
    implicitHeight: state === "clipboard" ? Math.min(580, (104 + spacing) * Math.max(1, count) - spacing + (expandedClipId >= 0 ? 228 : 0)) : (Tokens.sizes.launcher.itemHeight + spacing) * Math.min(Config.launcher.maxShown, count) - spacing

    StyledText {
        anchors.centerIn: parent
        visible: root.state === "clipboard" && root.count === 0
        text: Clipboard.loading ? "Loading clipboard…" : (Clipboard.transformSearch(root.search.text).trim() ? "No matching clipboard items" : "Copy text or an image to start your history")
    ...
    Connections {
        function onLauncherChanged() {
            root.syncDisplayText();
            if (root.screenState.launcher && root.requestedState === "clipboard")
                Clipboard.reload();
        }
