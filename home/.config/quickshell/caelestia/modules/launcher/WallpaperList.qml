...
PathView {
    ...
    readonly property real thumbnailWidth: Math.min(Tokens.sizes.launcher.wallpaperWidth * 1.3, Math.max(160, ((QsWindow.window as QsWindow)?.screen.width ?? 1920) - 160))
    readonly property int itemWidth: thumbnailWidth * 0.88 + Tokens.padding.medium * 2
    implicitHeight: thumbnailWidth * 9 / 16 + Tokens.padding.large + Tokens.padding.medium + Tokens.font.label.medium.pixelSize + Tokens.spacing.extraSmall + Tokens.spacing.small + 18
    ...
    model: ScriptModel {
        ...
        onValuesChanged: root.currentIndex = search ? 0 : Math.max(0, values.findIndex(w => w.path === Wallpapers.actualCurrent))
    }

    Component.onCompleted: currentIndex = Math.max(0, scriptModel.values.findIndex(w => w.path === Wallpapers.actualCurrent))
