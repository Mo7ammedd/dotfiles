...
RippleButton {
    ...
    property int buttonHorizontalPadding: 14
    property int buttonVerticalPadding: 10
    property bool keyboardDown: false
    readonly property bool selected: root.highlighted || root.hovered || root.activeFocus

    implicitHeight: Math.max(64, rowLayout.implicitHeight + root.buttonVerticalPadding * 2)
    implicitWidth: rowLayout.implicitWidth + root.buttonHorizontalPadding * 2
    buttonRadius: Appearance.rounding.small
    colBackground: (root.down || root.keyboardDown) ? Appearance.colors.colPrimaryContainerActive : 
        (selected ? ColorUtils.mix(Appearance.m3colors.m3surfaceContainer, Appearance.colors.colPrimary, 0.88) : "transparent")
    colBackgroundHover: ColorUtils.mix(Appearance.m3colors.m3surfaceContainer, Appearance.colors.colPrimary, 0.88)
    colRipple: Appearance.colors.colPrimaryContainerActive
    property color colForeground: selected ? Appearance.colors.colOnPrimaryContainer : Appearance.m3colors.m3onSurface

    readonly property string highlightPrefix: `<b><font color="${Appearance.colors.colPrimary}">`
    readonly property string highlightSuffix: `</font></b>`
    ...
    Rectangle {
        ...
        color: "transparent"
        border.width: 1
        border.color: root.selected ? ColorUtils.transparentize(Appearance.colors.colPrimary, 0.72) : "transparent"
    }

    onClicked: {
        GlobalStates.overviewOpen = false
        if (root.itemExecute) root.itemExecute()
    }
    ...
    RowLayout {
        id: rowLayout
        spacing: iconLoader.sourceComponent === null ? 0 : 14
        anchors.fill: parent
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Repeater { // Favicons for links
                    ...
                    Favicon {
                        ...
                        size: parent?.height ?? Appearance.font.pixelSize.normal
                        url: modelData
                    }
                }
                StyledText { // Item name/content
                    ...
                    font.variableAxes: ({ "wght": root.selected ? 600 : 450 })
            ...
            StyledText {
                Layout.fillWidth: true
                visible: root.itemType === Translation.tr("App")
                text: root.entry?.genericName || root.entry?.comment || Translation.tr("Application")
                textFormat: Text.PlainText
                font.pixelSize: 12
                color: root.selected ? Appearance.colors.colOnSurfaceVariant : Appearance.colors.colSubtext
                elide: Text.ElideRight
                maximumLineCount: 1
            }
        ...
        StyledText {
            ...
            visible: root.selected && root.itemClickActionName !== ""
            id: clickAction
            font.pixelSize: 12
